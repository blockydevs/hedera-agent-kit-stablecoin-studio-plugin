...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    Role: { buildResetAllowance: vi.fn() },
    ResetSupplierAllowanceRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { RESET_SUPPLIER_ALLOWANCE_TOOL } from '@/tools/lifecycle/reset-supplier-allowance';

const makeClient = () => Client.forNetwork({});

describe('reset-supplier-allowance tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(RESET_SUPPLIER_ALLOWANCE_TOOL);
    expect(tool.name).toBe('Reset Supplier Allowance');
  });

  it('should reset supplier allowance successfully', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (Role.buildResetAllowance as any).mockResolvedValue({
    ...
    expect(Role.buildResetAllowance).toHaveBeenCalled();
