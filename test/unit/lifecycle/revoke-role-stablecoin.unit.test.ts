...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    Role: { buildRevokeRole: vi.fn() },
    RevokeRoleRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { REVOKE_ROLE_STABLECOIN_TOOL } from '@/tools/lifecycle/revoke-role-stablecoin';

const makeClient = () => Client.forNetwork({});

describe('revoke-role-stablecoin tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(REVOKE_ROLE_STABLECOIN_TOOL);
    expect(tool.name).toBe('Revoke Role');
  });

  it('should revoke role successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    const fakeResponse = {
      ...
      humanMessage: 'Successfully revoked role from account for stablecoin.',
    };
    ...
    expect(Role.buildRevokeRole).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (Role.buildRevokeRole as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', targetId: '0.0.6666', role: 'WIPE_ROLE' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to revoke role: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
