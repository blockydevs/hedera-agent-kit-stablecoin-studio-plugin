...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildUpdateReserveAddress: vi.fn() },
    UpdateReserveAddressRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { UPDATE_RESERVE_ADDRESS_STABLECOIN_TOOL } from '@/tools/lifecycle/update-reserve-address';

const makeClient = () => Client.forNetwork({});

describe('update-reserve-address tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(UPDATE_RESERVE_ADDRESS_STABLECOIN_TOOL);
    expect(tool.name).toBe('Update Reserve Address');
  });

  it('should update reserve address successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildUpdateReserveAddress as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully updated reserve address for stablecoin.',
    };
    (handleTransaction as any).mockResolvedValue(fakeResponse);

    const params = { tokenId: '0.0.5555', reserveAddress: '0.0.6666' };
    const res: any = await tool.execute(client, autonomousContext, params);

    expect(res).toEqual(fakeResponse);
    expect(StableCoin.buildUpdateReserveAddress).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildUpdateReserveAddress as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', reserveAddress: '0.0.6666' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to update reserve address: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
