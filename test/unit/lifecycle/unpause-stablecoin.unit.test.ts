...
describe('unpause-stablecoin tool (unit)', () => {
  ...
  it('should unpause stablecoin successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildUnPause as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully unpaused stablecoin.',
    };
    ...
    expect(StableCoin.buildUnPause).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildUnPause as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to unpause stablecoin: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
