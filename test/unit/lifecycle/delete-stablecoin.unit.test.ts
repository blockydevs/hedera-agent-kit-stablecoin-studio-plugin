...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildDelete: vi.fn() },
    DeleteRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { DELETE_STABLECOIN_TOOL } from '@/tools/lifecycle/delete-stablecoin';

const makeClient = () => Client.forNetwork({});

describe('delete-stablecoin tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(DELETE_STABLECOIN_TOOL);
    expect(tool.name).toBe('Delete Stablecoin');
  });

  it('should delete stablecoin successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildDelete as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully deleted stablecoin.',
    };
    ...
    expect(StableCoin.buildDelete).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildDelete as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to delete stablecoin: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
