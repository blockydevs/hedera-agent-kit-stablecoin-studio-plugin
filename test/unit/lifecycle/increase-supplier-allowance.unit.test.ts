...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    Role: { buildIncreaseAllowance: vi.fn() },
    IncreaseSupplierAllowanceRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { INCREASE_SUPPLIER_ALLOWANCE_TOOL } from '@/tools/lifecycle/increase-supplier-allowance';

const makeClient = () => Client.forNetwork({});

describe('increase-supplier-allowance tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(INCREASE_SUPPLIER_ALLOWANCE_TOOL);
    expect(tool.name).toBe('Increase Supplier Allowance');
  });

  it('should increase supplier allowance successfully', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    const params = { tokenId: '0.0.5555', targetId: '0.0.6666', amount: '100' };
    const res: any = await tool.execute(client, autonomousContext, params);

    expect(res).toEqual(fakeResponse);
    expect(Role.buildIncreaseAllowance).toHaveBeenCalled();
