...
import toolFactory, { ASSOCIATE_STABLECOIN_TOOL } from '@/tools/account/associate-stablecoin';

const makeClient = () => Client.forNetwork({});

describe('associate-stablecoin tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(ASSOCIATE_STABLECOIN_TOOL);
    expect(tool.name).toBe('Associate Stablecoin');
  });

  it('should associate stablecoin successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildAssociate as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully associated stablecoin with account.',
    };
    ...
    expect(StableCoin.buildAssociate).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildAssociate as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to associate stablecoin: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
