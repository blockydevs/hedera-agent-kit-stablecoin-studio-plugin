...
import toolFactory, { FREEZE_ACCOUNT_TOOL } from '@/tools/account/freeze-account';

const makeClient = () => Client.forNetwork({});

describe('freeze-account tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(FREEZE_ACCOUNT_TOOL);
    expect(tool.name).toBe('Freeze Account');
  });

  it('should freeze account successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildFreeze as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully froze account for stablecoin.',
    };
    ...
    expect(StableCoin.buildFreeze).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildFreeze as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', targetId: '0.0.6666' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to freeze account: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
