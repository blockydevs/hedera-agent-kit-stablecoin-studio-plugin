...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildRevokeKyc: vi.fn() },
...
import toolFactory, { REVOKE_KYC_TOOL } from '@/tools/account/revoke-kyc';

const makeClient = () => Client.forNetwork({});

describe('revoke-kyc tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(REVOKE_KYC_TOOL);
    expect(tool.name).toBe('Revoke KYC');
  });

  it('should revoke KYC successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildRevokeKyc as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully revoked KYC from account for stablecoin.',
    };
    ...
    expect(StableCoin.buildRevokeKyc).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildRevokeKyc as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', targetId: '0.0.6666' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to revoke KYC: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
