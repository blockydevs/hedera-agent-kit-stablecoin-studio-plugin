...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildUnFreeze: vi.fn() },
...
import toolFactory, { UNFREEZE_ACCOUNT_TOOL } from '@/tools/account/unfreeze-account';

const makeClient = () => Client.forNetwork({});

describe('unfreeze-account tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(UNFREEZE_ACCOUNT_TOOL);
    expect(tool.name).toBe('Unfreeze Account');
  });

  it('should unfreeze account successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildUnFreeze as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully unfroze account for stablecoin.',
    };
    ...
    expect(StableCoin.buildUnFreeze).toHaveBeenCalled();
  });
  ...
  it('should return transaction in return-bytes mode', async () => {
    ...
    const params = { tokenId: '0.0.123456', accountId: '0.0.789012' };
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildUnFreeze as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', targetId: '0.0.6666' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to unfreeze account: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
