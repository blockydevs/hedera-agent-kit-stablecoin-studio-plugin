...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildWipe: vi.fn() },
    WipeRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { WIPE_STABLECOIN_TOOL } from '@/tools/supply/wipe-stablecoin';

const makeClient = () => Client.forNetwork({});

describe('wipe-stablecoin tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(WIPE_STABLECOIN_TOOL);
    expect(tool.name).toBe('Wipe Stablecoin');
  });

  it('should wipe tokens successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildWipe as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully wiped tokens for stablecoin.',
    };
    ...
    expect(StableCoin.buildWipe).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildWipe as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', amount: '100', targetId: '0.0.6666' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to wipe stablecoin: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
