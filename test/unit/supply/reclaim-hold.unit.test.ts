...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildReclaimHold: vi.fn() },
    ReclaimHoldRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { RECLAIM_HOLD_STABLECOIN_TOOL } from '@/tools/supply/reclaim-hold';

const makeClient = () => Client.forNetwork({});

describe('reclaim-hold tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(RECLAIM_HOLD_STABLECOIN_TOOL);
    expect(tool.name).toBe('Reclaim Hold');
  });

  it('should reclaim hold successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildReclaimHold as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully reclaimed hold for stablecoin.',
    };
    (handleTransaction as any).mockResolvedValue(fakeResponse);

    const params = { tokenId: '0.0.5555', holdId: 123, sourceId: '0.0.6666' };
    const res: any = await tool.execute(client, autonomousContext, params);

    expect(res).toEqual(fakeResponse);
    expect(StableCoin.buildReclaimHold).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildReclaimHold as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', holdId: 123, sourceId: '0.0.6666' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to reclaim hold: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
