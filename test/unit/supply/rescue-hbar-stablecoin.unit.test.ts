...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildRescueHBAR: vi.fn() },
    RescueHBARRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { RESCUE_HBAR_STABLECOIN_TOOL } from '@/tools/supply/rescue-hbar-stablecoin';

const makeClient = () => Client.forNetwork({});

describe('rescue-hbar-stablecoin tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(RESCUE_HBAR_STABLECOIN_TOOL);
    expect(tool.name).toBe('Rescue HBAR Stablecoin');
  });

  it('should rescue HBAR successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildRescueHBAR as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully rescued HBAR for stablecoin.',
    };
    (handleTransaction as any).mockResolvedValue(fakeResponse);

    const params = { tokenId: '0.0.5555', amount: '10' };
    const res: any = await tool.execute(client, autonomousContext, params);

    expect(res).toEqual(fakeResponse);
    expect(StableCoin.buildRescueHBAR).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildRescueHBAR as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', amount: '10' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to rescue HBAR from stablecoin: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
