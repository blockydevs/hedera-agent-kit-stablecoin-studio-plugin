...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildReleaseHold: vi.fn() },
    ReleaseHoldRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { RELEASE_HOLD_STABLECOIN_TOOL } from '@/tools/supply/release-hold';

const makeClient = () => Client.forNetwork({});

describe('release-hold tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(RELEASE_HOLD_STABLECOIN_TOOL);
    expect(tool.name).toBe('Release Hold');
  });

  it('should release hold successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildReleaseHold as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully released hold for stablecoin.',
    };
    ...
    expect(StableCoin.buildReleaseHold).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildReleaseHold as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', holdId: 123, amount: '100', sourceId: '0.0.6666' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to release hold: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
