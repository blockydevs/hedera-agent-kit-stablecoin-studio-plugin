...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildRescue: vi.fn() },
    RescueRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { RESCUE_STABLECOIN_TOOL } from '@/tools/supply/rescue-stablecoin';

const makeClient = () => Client.forNetwork({});

describe('rescue-stablecoin tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(RESCUE_STABLECOIN_TOOL);
    expect(tool.name).toBe('Rescue Stablecoin');
  });

  it('should rescue tokens successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildRescue as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully rescued tokens for stablecoin.',
    };
    ...
    expect(StableCoin.buildRescue).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildRescue as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', amount: '100' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to rescue stablecoin: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
