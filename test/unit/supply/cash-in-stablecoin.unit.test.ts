...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => {
  return {
    ...
    StableCoin: { buildCashIn: vi.fn() },
    CashInRequest: class { constructor(x: any) { Object.assign(this, x); } },
...
import toolFactory, { CASH_IN_STABLECOIN_TOOL } from '@/tools/supply/cash-in-stablecoin';

const makeClient = () => Client.forNetwork({});

describe('cash-in-stablecoin tool (unit)', () => {
  ...
  it('should expose the correct method and name', () => {
    ...
    expect(tool.method).toBe(CASH_IN_STABLECOIN_TOOL);
    expect(tool.name).toBe('Cash In Stablecoin');
  });

  it('should cash in stablecoin successfully in autonomous mode', async () => {
    const tool = toolFactory(autonomousContext, config);
    ...
    (StableCoin.buildCashIn as any).mockResolvedValue({ serializedTransaction: fakeTxBytes });
    
    const fakeResponse = {
      raw: { transactionId: '0.0.1001@123.456', status: Status.Success },
      humanMessage: 'Successfully cashed in stablecoin.',
    };
    (handleTransaction as any).mockResolvedValue(fakeResponse);

    const params = { tokenId: '0.0.5555', amount: '100', targetId: '0.0.6666' };
    const res: any = await tool.execute(client, autonomousContext, params);

    expect(res).toEqual(fakeResponse);
    expect(StableCoin.buildCashIn).toHaveBeenCalled();
  });
  ...
  it('should handle SDK errors', async () => {
    ...
    (StableCoin.buildCashIn as any).mockRejectedValue(new Error('SDK Error'));

    const params = { tokenId: '0.0.5555', amount: '100', targetId: '0.0.6666' };
    const res = await tool.execute(client, autonomousContext, params);

    expect(res.humanMessage).toContain('Failed to cash in (mint) stablecoin: SDK Error');
    expect(res.raw.status).toBe(Status.InvalidTransaction);
  });
});
