...
vi.mock('@hashgraph/stablecoin-npm-sdk', () => ({
  ...
  StableCoin: { delete: vi.fn() },
  DeleteRequest: vi.fn().mockImplementation(function (args) { Object.assign(this, args); }),
}));
...
describe('delete-stablecoin tool (unit)', () => {
  ...
  it('executes AUTONOMOUS happy path', async () => {
    ...
    (StableCoin.delete as any).mockResolvedValue({ status: 'SUCCESS' });

    const res: any = await tool.execute(client, context, { tokenId: '0.0.5555' });

    expect(res.raw).toEqual({ status: 'SUCCESS' });
    expect(res.humanMessage).toContain('deleted successfully');
    expect(res.humanMessage).toContain('irreversible');
  ...
  it('returns error when AUTONOMOUS mode without privateKey', async () => {
    ...
    expect(res.humanMessage).toContain('Failed to delete stablecoin');
  ...
  it('returns error response when SDK throws', async () => {
    ...
    const { StableCoin } = await import('@hashgraph/stablecoin-npm-sdk');
    (StableCoin.delete as any).mockRejectedValue(new Error('treasury not empty'));

    const res = await tool.execute(client, context, { tokenId: '0.0.5555' });
    expect(res.humanMessage).toContain('Failed to delete stablecoin');
    expect(res.humanMessage).toContain('treasury not empty');
  });
});
