...
const getSupplierAllowancePrompt = (context: Context = {}) => {
  ...
  return `
${contextSnippet}
Retrieves the minting allowance for a specific account (supplier) for a stablecoin.
...
IMPORTANT: When reporting the allowance amount, do NOT add thousand separators (commas). For example, say "1300" instead of "1,300".
${usageInstructions}
`;
};
...
const getSupplierAllowanceParameters = (context: Context = {}) => {
  ...
  return z.object({
    ...
    targetId: z
      ...
      .default(accountId)
      .describe(
        `The Hedera account ID to check the allowance for (e.g., "0.0.789012"). Default: ${accountId}`,
      ),
  });
};
