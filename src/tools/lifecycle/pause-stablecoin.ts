...
const pauseStablecoinPrompt = (context: Context = {}) => {
  ...
  return `
${contextSnippet}
Pauses a stablecoin, preventing all token transfers while preserving balances and ownership. Requires the pause role.
...
export class PauseStablecoinTool extends BaseTool {
  ...
  async coreAction(request: PauseRequest, _context: Context, _client: Client) {
    const response: SerializedTransactionData = await StableCoin.buildPause(request);
